...
domain from [Fraud Detection](../fraud_detection/). The framework code is
the same; only the data shape, the pipeline, and the metric names differ.

This case study is the **reusability proof** required by Week 4 of the
project: a non-fraud, non-ML-library, non-default-schema app uses the
exact same SDK and gets the same guarantees (datasets, versions, runs,
lineage, models, governance).
